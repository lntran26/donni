...
def plot_by_param_log(true, pred, log, ax, r2=None, case=None, vals=None):
    ...
    if r2:
        x, y = ax_min + 0.2, ax_max - 0.2
        if log:
            x = 10**(math.log(ax_min, 10) + 0.2)
            y = 10**(math.log(ax_max, 10) - 0.2)
        ax.text(x, y, "R^2: " + str(round(r2, 3)), fontsize=20)
...
def nn_train(train_dict, nn=None, solver='adam'):
    ...
    if nn is None:
        ...
        nn = MLPRegressor(solver=solver, max_iter=400)
