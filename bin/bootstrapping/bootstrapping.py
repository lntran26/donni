...
theta_list = [10000, 1000, 100] # global variable
...
def bootstrap_predictions(ml_model, bs_data_file, name):
    '''
    Runs the ML predictions for all of the generated bootstrap samples and saves
    to the results folder.
    ml_model: the trained ML model to use for predictions.
    bs_data_file: path to the bootstrap data.
    name: desired name of the file, to be appended to saveto/bootstrap_ (e.g., if
    name is 'mlpr_1000', will save to 'saveto/bootstrap_mlpr_1000')
    '''
    ...
    pickle.dump(pred_dict_list, open(f'{save_to}bootstrap_{name}', 'wb'), 2)
...
def plot_distribution(datafile, params, theta, n):
    ...
    size = len(params)
    fig, axs = plt.subplots(size, figsize=(5, size*2))
    for i in range(size):
        true = p_orig[i]
    ...
    handles, labels = axs[size-1].get_legend_handles_labels()
